...
type
  ...
  private
    FStartColor: TColor;
    FEndColor: TColor;
    FEnabled: Boolean; // Novo: para ativar/desativar o gradiente
    FOnChange: TNotifyEvent;
    procedure SetStartColor(const Value: TColor);
    procedure SetEndColor(const Value: TColor);
    procedure SetEnabled(const Value: Boolean); // Novo
  protected
  ...
  published
    property Enabled: Boolean read FEnabled write SetEnabled default True; // Novo
  ...
  private
    FOnChange: TNotifyEvent;
    FRadius: Integer;
    FBorderColor: TColor; // Usado se UseGradientBorder = False
    FBorderThickness: Integer;
    FBorderStyle: TPenStyle;
    FUseGradientBorder: Boolean; // Novo: para usar borda gradiente
    FGradient: TFLUIGradientColor; // Novo: gradiente específico para a borda

    procedure SetRadius(const Value: Integer);
    procedure SetBorderColor(const Value: TColor);
    procedure SetBorderThickness(const Value: Integer);
    procedure SetBorderStyle(const Value: TPenStyle);
    procedure SetUseGradientBorder(const Value: Boolean); // Novo
    procedure SetGradient(const Value: TFLUIGradientColor); // Novo
  protected
    procedure Changed; virtual;
    procedure GradientChanged(Sender: TObject); // Novo
  public
    constructor Create;
    destructor Destroy; override; // Novo: para liberar FGradient
    procedure Assign(Source: TPersistent); override;
  published
    ...
    property UseGradientBorder: Boolean read FUseGradientBorder write SetUseGradientBorder default False; // Novo
    property Gradient: TFLUIGradientColor read FGradient write SetGradient; // Novo
    property OnChange: TNotifyEvent read FOnChange write FOnChange;
  end;
  ...
  private
    FOnChange: TNotifyEvent;
    FFillColor: TColor; // Usado se Gradient.Enabled = False ou ButtonType não é fbtGradient
    FGradient: TFLUIGradientColor; // Gradiente para o preenchimento
  ...
  protected
    procedure Changed; virtual;
    procedure FillGradientChanged(Sender: TObject); // Renomeado para clareza
  public
    constructor Create;
...
begin
  inherited Create;
  FEnabled := True; // Gradientes são ativos por padrão
  FStartColor := clWhite;
  FEndColor := clSilver;
end;
...
begin
  ...
  FGradient.OnChange := Self.GradientChanged; // Conecta o evento
end;
...
begin
  Changed; // Repassa a notificação se o gradiente da borda mudar
end;
...
begin
  ...
  FGradient := TFLUIGradientColor.Create;
  FGradient.OnChange := Self.FillGradientChanged; // Nome do handler atualizado
...
procedure TFLUIButtonColorSettings.FillGradientChanged(Sender: TObject); // Renomeado
begin
  Changed;
end;
...
begin
  FGradient.Assign(Value); // Este é o gradiente de preenchimento
end;
